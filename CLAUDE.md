...
- **`createRequestHandler(store)` is exported** (server startup is guarded by `require.main === module`) so the same handler can be reused as a Vercel serverless function without opening a port. `main()` `listen`s on `API_PORT` (**default `3001`** — `Number(process.env.API_PORT) || 3001`; root `.env` sets `API_PORT=3001`, and the dashboard's `STENION_API_URL` points at `:3001`, leaving `:3000` free for the dashboard itself).
...
## Dashboard / site — `@stenion/dashboard` (shipped — step 7 barebones, then full site build-out step 10)

- **Framework: Next.js 15, App Router** (React 19). Chosen over Vite because Vercel is the deploy target (first-party integration, zero-config), it gives Server Components for the fetch model below, and file-based routing maps 1:1 to the pages. Its own `tsconfig.json` (bundler resolution, `jsx: preserve`, `next` TS plugin) — **does not** extend `tsconfig.node.json`, per "TypeScript config".
- **Fetch model (settled): server-side.** _(⚠️ Partly SUPERSEDED at deploy 2026-08-12 — see "Deploy architecture": the dashboard no longer fetches a separate API over HTTP; `lib/api.ts` now reads `@stenion/db`'s Store in-process and `STENION_API_URL` is removed. The "server-side, never from the browser" principle still holds; the HTTP hop and `STENION_API_URL` details below are historical.)_ Data pages are async Server Components that fetch the API from Node (on Vercel), never from the browser. Consequences: (1) the API base URL is a **server-only** env var **`STENION_API_URL`** (no `NEXT_PUBLIC_` prefix — the browser never sees or calls the API). **Ports (actual, do not "fix"):** API runs on **`:3001`** (root `.env` sets `API_PORT=3001`; `api/src/index.ts` default is `|| 3001`), the dashboard runs on **`:3000`** (Next default), and `dashboard/.env.local` points `STENION_API_URL` at `http://localhost:3001`. Two different ports — there is **no** port conflict; earlier confusion came from concurrent servers sharing one `.next`, not config. (2) CORS is **not required** for the dashboard's own render path (server-to-server) — the API's CORS is purely for future direct/browser/third-party clients. Data routes are `ƒ` (dynamic, `force-dynamic` + `cache: 'no-store'`); About/Methodology are `○` static.
- **Styling stack (step 10, decided with maintainer via the session prompt — this DELIBERATELY overrides the old "no UI framework, hand-written CSS" house style for the dashboard only):**
  - **Tailwind CSS v4, CSS-first** (`@import "tailwindcss"` + `@theme` tokens in `app/globals.css`; PostCSS via `@tailwindcss/postcss`; **no `tailwind.config.js`**). Brand tokens live in `@theme`, consumed as utilities (`bg-bg`, `text-ink`, `text-accent`, `border-line`, `font-display`, …).
  - **Dark mode ONLY** — no toggle, no light-mode styles. One navy palette (`--color-bg #06090f` → layered `surface`/`surface-2`), off-white ink, and a deliberately **non-default accent pair**: custom aqua `--color-accent #3fd0c9` + periwinkle `--color-accent-2 #8b8ff5` (gradient wordmark/CTAs). The score ramp (`safe`/`warn`/`danger`) is a **separate semantic scale** — it encodes data, never reused as decoration. No `indigo-600`/`blue-600`, no generic `shadow-lg` cards — depth comes from `surface-lit` (top-lit gradient), fine `border-line` hairlines, and a dotted `grid-backdrop`.
  - **Fonts:** self-hosted `geist` package (GeistSans body + GeistMono for tabular numbers, zero network fetch) paired with **Space Grotesk** (display, via `next/font/google`) for large headings — a real pairing, not system-ui.
  - **Packages added:** `framer-motion` (mount/scroll reveals via `Reveal`/`RevealGroup`/`RevealItem`, animated factor bars, and the **count-up score ring**), `lucide-react` (all icons — no hand-rolled SVG), `react-markdown` + `remark-gfm` (on-site methodology). **shadcn/ui CLI intentionally NOT used** — this site has no overlay/Radix primitives to need it; instead we use its *pattern* (a `cn()` helper in `app/lib/cn.ts` via `clsx` + `tailwind-merge`, plus small styled components). Documented deviation, lighter for a pre-funding project.
  - **The score as the product:** `components/score-ring.tsx` renders the `safetyScore` as a radial gauge whose arc sweeps and figure counts up on mount (framer-motion; honors `prefers-reduced-motion`) — reinforcing "live, not static." Large tabular figures throughout.
- **Pages / structure** (nav is consistent across all via `app/layout.tsx` → `components/nav.tsx` + `components/footer.tsx`; nav = Home, Registry, Methodology, About, GitHub icon):
  - `app/page.tsx` — **Homepage**: hero ("Audits are a snapshot. Risk moves every block."), then **live proof** (fetches `/protocols`, renders Blend/Kinetic score rings), "why continuous > static audits/TVL", the five-factor teaser, and the trust-rules band. Degrades gracefully if the API is down (pitch still stands).
  - `app/registry/page.tsx` — **Protocol registry** (the ranked view; the word "leaderboard" appears **nowhere** in UI/copy, per the prompt). Ranked by `safetyScore`, payment-blind, each row → detail. Empty + API-error states.
  - `app/methodology/page.tsx` — renders the repo-root **`METHODOLOGY.md` on-site** (`react-markdown` + `remark-gfm`, `components/markdown-doc.tsx` adds heading slug ids + rewrites repo-relative links to GitHub) plus a "view source on GitHub" link. `next.config.mjs` `outputFileTracingIncludes` bundles the file for the route.
  - `app/about/page.tsx` — what Stenion is, continuous-vs-static, the four trust rules (payment-blind, AI explains-not-decides, no fabricated numbers, open-source adapters), honest-limits section.
  - `app/protocol/[id]/page.tsx` — redesigned detail: score-ring hero + `StatusPill`, five `FactorCard`s (animated bar + weight + real `detail` string), "Recent runs" history strip. Unknown id → `notFound()` → `app/not-found.tsx`.
  - **Loading states** on every fetching route (`app/loading.tsx`, `app/registry/loading.tsx`, `app/protocol/[id]/loading.tsx`) using shimmer `Skeleton`s, not spinners. **Error states** inline (API-unreachable notices), never a blank screen or leaked raw error.
- **Shared code:** `app/lib/api.ts` (typed fetch helpers + contract types, **duplicated from the HTTP contract, not imported from `@stenion/db`**), `app/lib/format.ts` (timestamp/staleness + band color/label helpers), `app/lib/cn.ts`, `app/lib/site.ts` (`NAV_LINKS` + `GITHUB_URL` = `github.com/stenion-lab/stenion` — the real org; the remote itself is not yet pushed, but the URL is settled and used site-wide).
- **Not yet deployed** (see step 8). `outputFileTracingRoot` is pinned to the repo root in `next.config.mjs`.
- **Run locally:** start the API on **:3001** (`node api/dist/index.js`, `API_PORT` defaults to 3001; needs `DATABASE_URL` + migrated tables), then `pnpm --filter @stenion/dashboard dev` (serves `:3000`, reads `.env.local` → API `:3001`). Build: `pnpm --filter @stenion/dashboard build`. **⚠️ Never run `next build`/`next start`/a second `next dev` against the same checkout while a dev server is up — they share one `.next` and corrupt each other (this bit us mid-session, produced phantom 500s). Vercel builds in isolation so this is a local-only hazard.**
- **Verified end-to-end against the live Neon DB (2026-08-11):** `next build` clean (5 routes: `/` `ƒ`, `/registry` `ƒ`, `/protocol/[id]` `ƒ`, `/about` + `/methodology` `○`); root `eslint .` + dashboard `tsc` clean. Live dev on `:3000` against API `:3001`: home shows Blend **54** + Kinetic **21** score rings, `/registry` ranks both, `/protocol/blend` + `/protocol/kinetic` render all factor `detail` strings, `/methodology` renders the full rulebook, `/about` renders, unknown id renders the 404 page (renders correct 404 UI; HTTP-status is 200 under `next dev` — a known dev-mode streamed-`notFound()` quirk, returns proper 404 in a production build).
...
## Deploy architecture (step 8 — merged into one Vercel project, 2026-08-12)

Decided with the maintainer: **one Vercel project = the `dashboard`.** The standalone `@stenion/api` and `indexer` are NOT deployed as separate Vercel services. Instead:

- **API merged into the dashboard as Next.js Route Handlers.** `@stenion/api`'s two endpoints now live at `dashboard/app/api/protocols/route.ts` (`GET /api/protocols`) and `dashboard/app/api/protocol/[id]/route.ts` (`GET /api/protocol/:id`). **Note the `/api` path prefix** (Next convention) — the old bare paths were `/protocols` and `/protocol/:id`. Both handlers reuse the **exact same `Store` methods** (`listProtocolsWithLatestScore`, `getProtocolDetail`) and return the **exact same JSON** as the shipped contract — a transport change (bare `node:http` → Route Handler), not a rewrite. Bare `node:http` doesn't run on Vercel's serverless model; one project is simpler to operate solo. Shared server-only helpers (Store singleton, CORS headers, `jsonResponse`) live in `dashboard/app/api/_shared.ts`. Both routes are `runtime = 'nodejs'`, `dynamic = 'force-dynamic'`. **`@stenion/api` is now legacy** — kept in the tree but not deployed; the live API is the dashboard routes.
- **The dashboard's own pages read the Store DIRECTLY, not over HTTP.** `dashboard/app/lib/api.ts` was refactored: it no longer fetches `STENION_API_URL` — it calls `@stenion/db`'s `Store` in-process (same methods the routes use) and is now `import 'server-only'`. **This deliberately reverses the old step-7 "lib/api is HTTP-decoupled" decision**, which only made sense when the API was a separate service; a server-side self-HTTP-call would need an absolute base URL and can be blocked by Vercel deployment protection on preview URLs, so a direct call is simpler and more robust. **`STENION_API_URL` is removed/unused.** The contract **types** were split into `dashboard/app/lib/contract.ts` (pure types, no `pg`) so client components (`factor-bar.tsx`) import types from there; `lib/api.ts` re-exports them for server components. The HTTP routes still exist for **external** consumers (wallets/third parties) — dashboard and routes both go through `Store`, so the contract can't drift.
- **Indexer is NOT deployed; it's triggered by a route.** `dashboard/app/api/cron/run-indexer/route.ts` (`POST`) runs **one** scoring cycle when hit. `indexer/src/index.ts` was refactored to export **`runIndexerCycle()`** (loads config, builds targets, upserts protocol metadata, runs one cycle, returns a `CycleSummary`; does **not** close the pool — warm-reuse under serverless), and its standalone interval loop is now guarded by `if (require.main === module)` (same pattern as the old API), so importing it for the route doesn't start the loop. `node dist/index.js` still runs the standalone looping indexer for local use.
- **Scheduling is EXTERNAL — GitHub Actions, not Vercel Cron.** Hobby-tier Vercel Cron is capped at once-per-day, too slow. A GitHub Actions workflow curls the cron route every ~5 min (the maintainer wires that up separately, outside this repo's runtime — not built this session).
- **Cron route is secret-gated.** It requires `Authorization: Bearer <CRON_SECRET>`, compared with `crypto.timingSafeEqual`. If `CRON_SECRET` is unset it **refuses to run (500)** — never open. No CORS on this route (not public). `maxDuration = 60` (a live 2-protocol cycle makes several on-chain sims; 60s is the Hobby max).

**CORS answer (maintainer asked):** the dashboard's own pages no longer need CORS at all — they read the Store in-process (same-origin/in-process, not cross-origin). CORS (`access-control-allow-origin: *`) is kept **only** on the two public data routes, purely for future external browser clients (wallets/third parties reading public, payment-blind data). The cron route has no CORS.

**Build wiring:**
- `dashboard/package.json` gained deps `@stenion/db` + `@stenion/indexer` (`workspace:*`); `@stenion/core`/`@stenion/adapters` come transitively. Its `build` script now compiles the workspace deps first: `pnpm --filter @stenion/core build && … @stenion/db … @stenion/adapters … @stenion/indexer build && next build` (chained with `&&` for cmd/sh portability — no fragile pnpm topo-filter globs). A `build:next` script runs `next build` alone. **On Vercel, set the Build Command to `pnpm run build`** so the deps compile before `next build` (the workspace packages resolve via `main: dist/…`, so their dist must exist at build+runtime).
- `dashboard/next.config.mjs` gained `serverExternalPackages: ['pg', '@stellar/stellar-sdk']` — keep those native/heavy deps as runtime requires instead of webpack-bundling them. `outputFileTracingRoot` (repo root) is retained so workspace-dep tracing is correct.

**Env vars (all on the ONE Vercel project — Production + Preview):** `DATABASE_URL` (Neon **pooled**), `STENION_RPC_URL`, `STENION_HORIZON_URL`, `CRON_SECRET`. `STENION_INTERVAL_MS`/`STENION_RUN_ONCE` are unused by the cron route. Documented in `.env.example` (root, with a "Deploying to Vercel" block) and `dashboard/.env.example`. **Local dev inherits all of these from the repo-root `.env` via the db/indexer walk-up loader — no `dashboard/.env.local` needed** (it's now essentially empty; `STENION_API_URL` removed from it). A random `CRON_SECRET` was added to the root `.env` for local cron testing.

**Verified locally (2026-08-12) against the live Neon DB, on the dev server (`:3002`):** `pnpm run build` clean (5 page routes + `/api/protocols`, `/api/protocol/[id]`, `/api/cron/run-indexer` all `ƒ`); dashboard `typecheck` + root `eslint .` clean. `GET /api/protocols` → Blend 54 + Kinetic 21 (contract intact); `GET /api/protocol/blend` → full factors; `GET /api/protocol/nonesuch` → 404; `OPTIONS` → 204 + CORS headers; cron `POST` with no/wrong secret → 401, with the correct `Bearer` secret → 200 `{triggered, ran:2, ok:2, failed:0}` and a **real live cycle persisted** to Neon; homepage renders. **⚠️ Known caveat:** a `next start` **production** server (`:3005`) 500'd on the cron route while a `next dev` was concurrently live on `:3002` — the documented `.next`-corruption hazard (concurrent build/dev share one `.next`). The build itself compiled the cron route without error, and dev serves it correctly, so this is corruption, not a real production defect. **Before deploy, do one clean `pnpm run build` with no dev server running to confirm; Vercel builds in isolation so it won't hit this.**

**What's left for the maintainer (manual — outward-facing, needs `vercel`/GitHub login):**
1. **GitHub remote.** Push the repo to `github.com/stenion-lab/stenion` (Vercel Git integration + the DeFiLlama-style contribution model both want a public repo). Or use `vercel deploy` from the CLI without Git.
2. **Vercel project.** New project → import the repo → **Root Directory = `dashboard`** → **Build Command = `pnpm run build`** (override the default so workspace deps compile) → Install Command = `pnpm install` (default). Framework preset: Next.js (auto).
3. **Env vars.** Paste `DATABASE_URL` (Neon pooled), `STENION_RPC_URL`, `STENION_HORIZON_URL`, `CRON_SECRET` into the project's Environment Variables (Production + Preview). Values come from the root `.env`.
4. **GitHub Actions cron.** Add a workflow that, every 5 min, `curl -X POST -H "Authorization: Bearer ${{ secrets.CRON_SECRET }}" https://<deploy-domain>/api/cron/run-indexer`. Store `CRON_SECRET` as a GitHub Actions secret (same value as Vercel).
5. **Domain.** Once stable on `.vercel.app`, connect `stenion.com`.
...
> **⚠️ EXECUTION ORDER (decided 2026-08-11, after going back and forth on priority):**
> **step 9 (more adapters) ✅ done → step 10 (dashboard/site build-out) ✅ done → step 8 (deploy) ← NOW**,
> in that order even though the step _numbers_ are out of order. Rationale: build **real
> multi-protocol data first**, so the homepage/positioning has genuine substance behind it
> (Blend + Kinetic) rather than one protocol propped up by marketing copy, and so deploy
> happens once against a site that's actually worth showing. **Steps 9 and 10 are both
> closed** — two real Soroban adapters shipped, and the full site (homepage/registry/
> methodology/about/nav, Tailwind v4 dark theme) is built on top of them and verified live.
> **Deploy (step 8) is the only remaining step** — it's an outward-facing action needing the
> maintainer's `vercel` login, so it was not done autonomously. The step numbers are
> historical (8 was written before this re-sequencing); trust this execution-order note, not
> the numeric order, if they disagree.

8. **← CURRENT — restructuring done, manual deploy pending (2026-08-12).** The codebase is now restructured for a **single Vercel project (`dashboard`)**: API merged into Next Route Handlers, indexer exposed via a secret-gated cron-trigger route, scheduling moved to external GitHub Actions. **Full detail in "Deploy architecture" above** (final decisions, env vars, CORS, build wiring, local verification, and the exact manual steps). The remaining work is the outward-facing deploy itself (GitHub remote + Vercel project + env vars + GH Actions cron + domain) — needs the maintainer's `vercel`/GitHub login, so not done autonomously. `outputFileTracingRoot` (repo root) is retained so Vercel traces workspace deps correctly. `@stenion/api` is legacy (not deployed); `STENION_API_URL` is removed.

9. ✅ **DONE (closed 2026-08-11).** More protocol adapters — real multi-protocol data before the site build-out. Outcome: **Blend + Kinetic shipped; YieldBlox and Templar both researched and deliberately skipped** (neither is a native-Soroban lending protocol Stenion can score trustlessly). Two real, independent Soroban protocols judged enough substance to proceed to step 10.
   ...
   - **Templar — NOT built as an adapter (finding, 2026-08-11).** Research (its own [`Templar-Protocol/contracts`](https://github.com/Templar-Protocol/contracts) repo + launch coverage) established Templar is **not a native-Soroban lending protocol** — it's a NEAR-based, chain-abstraction ("Cypher Lending") protocol whose **lending market state lives on NEAR**, not Stellar:
     - The `market`, `vault`, and `registry` crates are all `near_sdk` (confirmed: `templar-market-contract` → `near-sdk.workspace = true`). Reserves, supply/borrow, utilization, interest, snapshots, and collateral positions — everything the five factors measure — are **NEAR** contract state, read via NEAR RPC, not Soroban RPC.
     - Stellar is a **wallet + collateral entry point via NEAR MPC / Chain Signatures** (deposit XLM, borrow USDC). XLM collateral sits in MPC-controlled Stellar accounts; the lending logic runs on NEAR. Architecture is Morpho-vault-v2-style (they forked `morpho-org/vault-v2`, Solidity) — isolated curator-driven markets, not a Blend/Aave shared multi-reserve pool.
     - **The only native-Soroban contract Templar ships is a price oracle** (`contract/proxy-oracle/soroban/` — a SEP-40 proxy oracle + its own governance contract + `Sep40Adapter`s; `aggregated_latest`, `registered_assets`, `get_owner`). A recursive scan of the repo tree found `soroban_sdk` code **only** under `proxy-oracle/soroban`.
     - **Taxonomy mapping against Soroban-readable data:** only **1 of 5** factors (`oracleSafety`) is natively on Stellar, and it's the oracle *infrastructure*, not lending-market health. `collateralSafety`/`liquiditySafety`/`utilizationSafety`/`adminKeySafety` all require the NEAR market state. A faithful Templar adapter would need to read **NEAR** — which breaks Stenion's non-negotiable rule that adapters read directly from trustless Stellar infra (Soroban RPC + Horizon). The alternatives are worse: fabricating Soroban lending data that doesn't exist (violates "no fabricated numbers") or a 1-of-5-factor stub (violates the taxonomy).
     - **Maintainer decision (2026-08-11): skip Templar; move to the site build-out (step 10).** Two real, independent Soroban protocols (Blend + Kinetic) is enough substance. Templar could later be represented only if Stenion's model expands to read NEAR (a category change to the whole trustless-Stellar positioning) — tracked as a possible future item, **not** built now.
     - Same class of finding as YieldBlox (#1): a real, significant protocol that turns out not to be an independently-scoreable native-Soroban lending pool. Confirming this from its own contracts *before* writing scoring logic is exactly the point of the "don't assume it mirrors Blend/K2; confirm actual structure first" rule.
   - **Nectar Network — WATCH for mainnet (candidate future third adapter, 2026-08-11).** Flagged as the next protocol to evaluate once it's live on Stellar mainnet. **Not built now** — same discipline as above: confirm it's an independently-scoreable **native-Soroban** lending protocol (reserves/utilization/oracle readable via Soroban RPC + Horizon, not another Blend pool or another-chain deployment) from its own contracts before writing any adapter. Revisit when mainnet contracts exist.

10. ✅ **DONE (2026-08-11).** Dashboard/site build-out — homepage, Registry, on-site Methodology, About, consistent nav/footer, loading + error states on every fetching route — on top of the barebones step-7 dashboard, backed by real Blend + Kinetic data. **Tailwind v4 (dark-mode-only) + framer-motion + lucide-react + geist/Space Grotesk fonts + react-markdown**; deliberately non-generic dark theme (custom aqua/periwinkle accents, animated count-up score ring as the hero number). "Leaderboard" never appears in UI copy (it's "Registry"). Positioning/copy written _after_ the data existed. Builds + lints + typechecks clean; verified live on `:3000` against API `:3001`. Full detail in "Dashboard / site" section above. Next: step 8 (deploy).

Local git initialized; commit history tracks scaffold → interface/blend scaffold → Blend adapter (`*Safety` taxonomy) → indexer → Postgres storage (`@stenion/db`) → public API → dashboard barebones → Kinetic adapter (`46159e9`) as checkpoints. GitHub org/remote still intentionally not set up — local-only for now. **Uncommitted in the working tree (commit when ready):** the full step-10 site build-out (new `dashboard/` pages + `components/*` + Tailwind v4 setup, new deps in `dashboard/package.json`: framer-motion, lucide-react, geist, react-markdown, remark-gfm, tailwindcss v4 + `@tailwindcss/postcss`/`typography`, clsx, tailwind-merge) **plus the step-8 deploy restructure (2026-08-12):** new `dashboard/app/api/{protocols,protocol/[id],cron/run-indexer}/route.ts` + `app/api/_shared.ts` + `app/lib/contract.ts`; refactored `app/lib/api.ts` (direct Store) and `indexer/src/index.ts` (`runIndexerCycle` export + auto-run guard); `dashboard/package.json` deps (`@stenion/db`, `@stenion/indexer`) + build script; `next.config.mjs` `serverExternalPackages`; env-doc updates (`.env.example` ×2, `.env.local`); and these CLAUDE.md updates. `pnpm-lock.yaml` also changed.
